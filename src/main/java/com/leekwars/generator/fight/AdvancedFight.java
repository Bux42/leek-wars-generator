...
public class AdvancedFight extends Fight {
    ...
    public AdvancedFight(Generator generator) {
        this(generator, null);
    }

    public AdvancedFight(Generator generator, FightListener listener) {
        super(generator);
        ...
        if (!current.isDead()) {
            ...
            if (ai != null) {
                if (ai.isValid()) {
                    ...
                    if (getTurn() == 1) {
                        ((LeekLog) current.getLogs()).addSystemLog(LeekLog.SERROR, Error.INVALID_AI,
                                new String[] { "Invalid AI" });
                    }
    ...
    public int useChip(Entity caster, Cell target, Chip template) {
        ...
        if (template.getAttack().getEffectParametersByType(Effect.TYPE_SUMMON) != null) {
            ...
            ((EntityAI) caster.getAI()).addSystemLog(LeekLog.STANDARD, Error.HELP_PAGE_LINK,
                    new String[] { "summons" });
    ...
    public static int getFightType(int type) {
        if (type == TYPE_SOLO_GARDEN || type == TYPE_SOLO_CHALLENGE || type == TYPE_SOLO_TOURNAMENT
                || type == TYPE_SOLO_TEST) {
            return Fight.TYPE_SOLO;
        } else if (type == TYPE_FARMER_GARDEN || type == TYPE_FARMER_TOURNAMENT || type == TYPE_FARMER_CHALLENGE
                || type == TYPE_FARMER_TEST) {
    ...
    public void startFightAdvanced(boolean drawCheckLife) throws Exception {
