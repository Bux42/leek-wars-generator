...
import com.leekwars.generator.fight.AdvancedFight;
...
public class AdvancedGenerator extends Generator {

    private static final String TAG = AdvancedGenerator.class.getSimpleName();
    ...
    public Outcome runScenario(Scenario scenario, FightListener listener, RegisterManager registerManager,
        ...
        AdvancedFight fight = new AdvancedFight(this, listener);
        ...
        try {
            ...
            for (var entity : fight.getState().getEntities().values()) {
                if (!entity.isSummon() && entity.getRegisters() != null
                    ...
                    registerManager.saveRegisters(entity.getId(), entity.getRegisters().toJSONString(),
        ...
        try {
            ...
            for (var entry : weapons.properties()) {
                ...
                Weapons.addWeapon(new Weapon(weapon.get("item").intValue(), weapon.get("cost").intValue(),
                        weapon.get("min_range").intValue(), weapon.get("max_range").intValue(),
                        (ArrayNode) weapon.get("effects"),
                        (byte) weapon.get("launch_type").intValue(), (byte) weapon.get("area").intValue(),
                        weapon.get("los").booleanValue(),
                        weapon.get("template").intValue(), weapon.get("name").asString(),
                        (ArrayNode) weapon.get("passive_effects"), weapon.get("max_uses").intValue()));
            }
    ...
    private void loadChips() {
        try {
            ...
            for (var entry : chips.properties()) {
                ...
                // System.out.println("New chip " + chip.get("name").asString() + " " + id + " "
                // + chip.get("template").intValue());
                Chips.addChip(
                        new Chip(Integer.parseInt(id), chip.get("cost").intValue(), chip.get("min_range").intValue(),
                                chip.get("max_range").intValue(), (ArrayNode) chip.get("effects"),
                                (byte) chip.get("launch_type").intValue(),
                                (byte) chip.get("area").intValue(), chip.get("los").booleanValue(),
                                chip.get("cooldown").intValue(),
                                chip.get("team_cooldown").booleanValue(), chip.get("initial_cooldown").intValue(),
                                chip.get("level").intValue(),
                                chip.get("template").intValue(), chip.get("name").asString(),
                                ChipType.values()[chip.get("type").intValue()], chip.get("max_uses").intValue()));
            }
    ...
    private void loadComponents() {
        try {
            ...
            for (var entry : components.properties()) {
                ...
                Components.addComponent(new Component(Integer.parseInt(id), component.get("name").asString(),
                        (ArrayNode) component.get("stats"), component.get("template").intValue()));
            }
     ...
     * try {
     * AI ai = LeekScript.compileFileContext(file,
     * "com.leekwars.generator.fight.entity.EntityAI", context, false);
